...
class Article extends react__WEBPACK_IMPORTED_MODULE_1__["Component"] {
  constructor(props) {
    ...
    this.ref = _Firebase__WEBPACK_IMPORTED_MODULE_3__["default"].firestore().collection('articles').where('title', '==', 'sad').get().then();
  ...
  componentDidMount() {
    const id = this.props.url.query.id;
    const ref = _Firebase__WEBPACK_IMPORTED_MODULE_3__["default"].firestore().collection('articles').where('title', '==', self.$route.params.id).get().then(doc => {
      if (doc.exists) {
        this.setState({
          board: doc.data(),
          key: doc.id,
...
//# sourceMappingURL=[id].js.69af933b9f3fd2ec53d4.hot-update.js.map